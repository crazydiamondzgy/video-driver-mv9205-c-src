...
#include <initguid.h>
DEFINE_GUID(GUID_MV9205, 
0x5665dec0, 0xa40a, 0x11d1, 0xb9, 0x84, 0x0, 0x20, 0xaf, 0xd7, 0x97, 0x84);
...
CAdapter::CAdapter()
{
	...
	KeInitializeTimer(&DMATimer[0]);
	KeInitializeTimer(&DMATimer[1]);
	KeInitializeTimer(&DMATimer[2]);
}

CAdapter::~CAdapter()
{
	KeCancelTimer(&DMATimer[0]);
	KeCancelTimer(&DMATimer[1]);
	KeCancelTimer(&DMATimer[2]);
...
NTSTATUS CAdapter::DispatchCreate(PDEVICE_OBJECT DeviceObject, PIRP Irp)
{
	...
	WRITE_REGISTER_DPGPRINT( PULONG(adapter->m_pBaseAddr + 0x258), 0x1007D3C0 );
	WRITE_REGISTER_DPGPRINT( PULONG(adapter->m_pBaseAddr + 0x2D8), 0x1007D3C0 );
	WRITE_REGISTER_DPGPRINT( PULONG(adapter->m_pBaseAddr + 0x358), 0x1007D3C0 );
	WRITE_REGISTER_DPGPRINT( PULONG(adapter->m_pBaseAddr + 0x3D8), 0x1007D3C0 );
...
BOOLEAN CAdapter::HwInterrupt()
{
   BOOLEAN fMyIRQ = FALSE; 
	...
	if (m_IRQExpected)
    {
		...
		DbgPrint("  Interrupt  - %8X\n", IntState.dw);
		if(m_bIRQError && ((IntState.dw&m_dwINTMaskBar) == 0 || IntState.dw != 0xFFFFFFFF ) )
		{
			fMyIRQ = TRUE; 
		}
		else
...
void CAdapter::cbDpcFuncErr0(IN PKDPC Dpc,IN CAdapter *pAdapter, IN PVOID SystemArg1, IN PVOID SystemArg2)
{
	...
	for(i=0; i<8; i++)
	{
		...
		{
			// Decoder Stop
			WRITE_REGISTER_DPGPRINT( PULONG(pAdapter->m_pBaseAddr + dwBase[i]), 0);//pAdapter->zz[i] );
		}
	}

	LARGE_INTEGER DueTime;
	DueTime.QuadPart = -100000L;
	BOOL bResult = KeSetTimer(
		&(pAdapter->DMATimer[0]),
		DueTime,
		&(pAdapter->m_IsrDpcErr[1])
		);
	DbgPrint("cbDpcFuncErr0 - Timer0 : %d - %8X - %8X - %8X - %8X\n", bResult, &(pAdapter->DMATimer[0]), (pAdapter->DMATimer[0]), &(pAdapter->m_IsrDpcErr[1]), (pAdapter->m_IsrDpcErr[1]));
}
...
//			WRITE_REGISTER_DPGPRINT( PULONG(pAdapter->m_pBaseAddr + dwRisc[i] + 0x10), 0x00000000 );
	...
	DbgPrint("cbDpcFuncErr1 - Timer1 : %d - %8X - %8X - %8X - %8X\n", bResult, &(pAdapter->DMATimer[1]), (pAdapter->DMATimer[1]), &(pAdapter->m_IsrDpcErr[2]), (pAdapter->m_IsrDpcErr[2]));
}
...
/*	for(i=0; i<8; i++)
	{
		...
		{
			// Decoder Stop
			WRITE_REGISTER_DPGPRINT( PULONG(pAdapter->m_pBaseAddr + dwBase[i]), 0);//pAdapter->m_dwDecDisable[i] );
		}
	}
	for(i=0; i<8; i++)
	{
	...
	for(i=0; i<8; i++)
	{
		DbgPrint("cbDpcFuncErr0 - m_dwDecEnable[%d]  = %8X\n", i, pAdapter->m_dwDecEnable[i]);
		//DbgPrint("cbDpcFuncErr0 - m_dwDecDisable[%d] = %8X\n", i, pAdapter->m_dwDecDisable[i]);
	}
*/
	...
	//DbgPrint("cbDpcFuncErr0 - DMA On [%X] : -1\n", pAdapter->m_IsrDpcErr[3]);
	//DbgPrint("cbDpcFuncErr0 - DMA On [%X] : -1\n", pAdapter->DMATimer[2]);
	...
	DbgPrint("cbDpcFuncErr2 - Timer2 : %d - %8X - %8X - %8X - %8X\n", bResult, &(pAdapter->DMATimer[2]), (pAdapter->DMATimer[2]), &(pAdapter->m_IsrDpcErr[3]), (pAdapter->m_IsrDpcErr[3]));
	//DbgPrint("cbDpcFuncErr0 - DMA On [%X] : -2\n", pAdapter->m_IsrDpcErr[3]);
	//DbgPrint("cbDpcFuncErr0 - DMA On [%X] : -2\n", pAdapter->DMATimer[2]);
}
...
{
	DbgPrint("  VideoCaptureRoutine - %8X\n", dwStreamNumber);
	//if(dwStreamNumber ==0)
...
NTSTATUS CAdapter::SetVideoFormat(PMV9205_FORMAT pFormat)
{
	...
	for(i=0; i<4; i++)
	{
		m_nSplit[i] = pFormat->nSplit[i];

		if(! pFormat->bEnable[i])
			continue;
		for(j=0; j<4; j++)
		{
			if(pFormat->nSplit[i])
		...
		case VIDEOFORMAT_YUY2:
			m_dwLinePitch[i] = pFormat->nWidth[i] * 2;
			break;
		case VIDEOFORMAT_YUV420:
		case VIDEOFORMAT_YUV411:
			m_dwLinePitch[i] = pFormat->nWidth[i] * 12 /8;
			break;
		case VIDEOFORMAT_Y8:
			m_dwLinePitch[i] = pFormat->nWidth[i];
			break;
		}

		//FramePerSec check
		if(pFormat->nFrame[i] < 100)
		{
			if(pFormat->nFrame[i] < 1)
				pFormat->nFrame[i] = 1;
			if(pFormat->nFrame[i] > 30)
				pFormat->nFrame[i] = 30;
		}
		else
		{
			if(pFormat->nFrame[i] < 101)
				pFormat->nFrame[i] = 101;
			if(pFormat->nFrame[i] > 125)
				pFormat->nFrame[i] = 125;
		}
		...
		dwReg10 &= 0xFFC00000;
		dwReg10 |= pFormat->nWidth[i];			//Even Field Image width Size
		dwReg10 |= (pFormat->nWidth[i] << 11);	//Odd  Field Image width Size
		...
		dwReg14 &= 0xFFC00000;
		if(pFormat->bInterlace[i])
		{
			dwReg14 |= pFormat->nHeight[i]/2;
			dwReg14 |= ((pFormat->nHeight[i]/2) << 11);
		}
		else
		{
			dwReg14 |= pFormat->nHeight[i];
			dwReg14 |= (pFormat->nHeight[i] << 11);
		}
		...
		if(pFormat->nSplit[i])
		{
			...
			m_pDstAddr[i][2] = m_pCaptureMemory[i] + pFormat->nHeight[i]*m_dwLinePitch[i]*2;
			m_pDstAddr[i][3] = m_pCaptureMemory[i] + pFormat->nHeight[i]*m_dwLinePitch[i]*2 + m_dwLinePitch[i];

			m_nHeight[i] = pFormat->nHeight[i];
		}
		else
		{
			dwReg1C = 0x00000300;
		...
		dwReg1C |= (DWORD(pFormat->nMux[i])<<29);
		...
		if(pFormat->nSplit[i])
		{
			...
			DWORD	dwDataSize;
			dwDataSize = pFormat->nHeight[i] * m_dwLinePitch[i];
		...
		//DbgPrint("\n\nCreateVideoRiscCmd\n  BOOL bInterlace = %d\n  int cx = %d\n  int cy = %d\n", pFormat->bInterlace[i], m_dwLinePitch[i], pFormat->nHeight[i]);
		//DbgPrint("  DWORD Split = %d\n  DWORD dwTAddr = %x\n  DWORD dwBaseAddr = %x\n \n", pFormat->nSplit[i], m_dwCapturePhysical[i], dwBase[i]);

		if(pFormat->nSplit[i] == 0)
		{
			if(pFormat->bStandard[i])
			{
				if(pFormat->nFrame[i] < 1)
					pFormat->nFrame[i] = 1;
				else if(pFormat->nFrame[i] > 25)
					pFormat->nFrame[i] = 25;

				char *pCh;
				pCh = FrameRate_PAL1[pFormat->nFrame[i]];
				for(j=0; j<25; j++)
				{
					int nFramecheck = j+i;
					if(nFramecheck>25)
					...
					{	// Skip
						CreateVideoRiscCmdNormal(pFormat->bInterlace[i], m_dwLinePitch[i], pFormat->nHeight[i], TRUE, &pProgLoc, m_dwCapturePhysical[i]);
					}
					else
					{
						CreateVideoRiscCmdNormal(pFormat->bInterlace[i], m_dwLinePitch[i], pFormat->nHeight[i], FALSE, &pProgLoc, m_dwCapturePhysical[i]);
					}
				}
			}
			else
			{	// NTSC
				if(pFormat->nFrame[i] < 1)
					pFormat->nFrame[i] = 1;
				else if(pFormat->nFrame[i] > 30)
					pFormat->nFrame[i] = 30;

				char *pCh;
				pCh = FrameRate_NTSC1[pFormat->nFrame[i]];
				for(j=0; j<30; j++)
				{
					int nFramecheck = j+i;
					if(nFramecheck>30)
		...
		{
			CreateVideoRiscCmdSplit(m_dwLinePitch[i], pFormat->nHeight[i], &pProgLoc, m_dwCapturePhysical[i], pFormat->nSplit[i], &m_pRiscVROPos[i], &m_dwDumyMemory[i]);
			RISC_JUMP(pProgLoc, ProgLocBase, 0);
...
NTSTATUS CAdapter::SetAudioFormat(PMV9205_AUDIOFM pAud)
{
	...
	for(i=0; i<4; i++)
	{
		if(pAud->bEnable[i])
		{
			...
			m_dwDecDisable[i+4] = m_dwDecEnable[i+4] & 0x7FFFFFFF;
